...
export function Progress() {
  ...
  const chartValues = [...chartWeights.map((weight) => weight.kg), ...(data.profile.targetKg === null ? [] : [data.profile.targetKg])];
  const min = Math.min(...chartValues) - 1;
  const max = Math.max(...chartValues) + 1;
  const chartY = (kg: number) => 128 - (kg - min) / (max - min) * 104;
